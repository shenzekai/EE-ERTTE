...
def process_test(batch_size, seq_data, model, FLAGS, epoch, is_test=False):
    ...
    pre_test_dataloader = DataLoaderX(pre_test_dataset, collate_fn=lambda x: collate_fn(x, FLAGS), batch_size=batch_size, num_workers=4)
    start_time = time.time()
...
parser.add_argument('--num_components', type=int, default=6779, help="number of Table components") # 5282 6779
