...
def collate_fn(data, FLAGS, model_type):
    # 根据不同架构设置对应的变量名
    ...
    if ext_attrs:
        for key in ext_attrs:
            dtype = torch.LongTensor if key in ['wide_index', 'deep_category'] else torch.FloatTensor
            attrs[key] = dtype(
                np.array([item[key] for item in data], dtype=np.float32 if dtype == torch.FloatTensor else np.int64))
